...
describe("FlatSequenceDesignTestSuite", () => {
    let viModel = null;
    let viDOM = null;
    let modelArr = null;
    let flatSeqArr = null;
    beforeAll( () => {
        ...
        modelArr = filterModelByType(viModel, "FlatSequence");
        viDOM = HTMLTools.getHTMLDOM(viModel);
        flatSeqArr = HTMLTools.filterByType(viDOM, "FlatSequence");
    });
    ...
    it("Given flat sequence model when div is created then div must have corresponding similar components", () => {
        for (let i = 0; i < flatSeqArr.length; i++) {
            ...
            const componentArr = modelArr[i].frameArr;
