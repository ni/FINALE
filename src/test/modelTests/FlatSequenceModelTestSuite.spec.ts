...
describe("FlatSequenceModelTestSuite", () => {
    let jsonObj = null;
    ...
    beforeAll( () => {
        ...
        structureObjArr = filterModelByType(niDocObj, "FlatSequence");
        jsonStructureObjArr = JSONTools.filterJSONByType(jsonObj.NIDocument, "FlatSequence");
    });

    it("given key id in Flat Sequence when Opened then ID must match JSON ID", () => {
        for (let i = 0; i < structureObjArr.length; i ++) {
            expect(structureObjArr[i].id).toBe(jsonStructureObjArr[i].ID);
        }
    });

    it("given key type in Flat Sequence when Opened then Type must match JSON Type", () => {
        for (let i = 0; i < structureObjArr.length; i ++) {
            expect(structureObjArr[i].type).toBe(jsonStructureObjArr[i].Type);
        }
    });

    it("given key components in Flat Sequence when Opened then component must match JSON Components", () => {
        for (let i = 0; i < structureObjArr.length; i ++) {
            const componentArr = structureObjArr[i].frameArr;
