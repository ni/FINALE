...
describe("StructureModelTestSuite", () => {
    let jsonObj = null;
    let structureObjArr = null;
    let jsonStructureObjArr = null;
    beforeAll( () => {
        jsonObj = JSONTools.fetchJSON("fibonacci\\fibonacci.json");
        const niDocObj = generateModel(jsonObj, "testAssets\\json");
        structureObjArr = filterModelByType(niDocObj, "Structure");
        jsonStructureObjArr = JSONTools.filterJSONByType(jsonObj.NIDocument, "Structure");
    });
